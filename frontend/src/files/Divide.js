...
const Divide = (props) => {
	...
	if(props.dndValues[props.num].type == 'vertical'){
		for(let i = 0; i < ((((max-props.hfValues.hfScrH) - (min - props.hfValues.hfScrH)) / props.constants.oneSM)/*Погрешность*/ - 1); i++){
		...
		for(let i = 0; i < (((max-props.hfValues.hfScrW) - (min - props.hfValues.hfScrW)) / props.constants.oneSM); i++){
